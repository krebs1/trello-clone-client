    ...
    <Draggable draggableId={`card:${card._id}`} index={index}>
      {
        provided => (
          <Box
            className='tw-max-w-full tw-pb-2'
            component='li'
          ...
          >
            <Paper
              className='tw-max-w-full tw-px-3 tw-pt-2 tw-pb-1 tw-rounded-lg'
              variant='elevation'
              elevation={10}
            >
              {
                (labels) &&
                  <Box
                      className='tw-grid tw-grid-cols-5 tw-gap-1 tw-w-full'
                  >
                    {
                      labels.map((elem) => (
                        <Tooltip
                          key={`${card._id}_${elem._id}`}
              ...
              }
              {
                card.name &&
                  <Typography
                      variant='h6'
                      className='tw-truncate tw-max-w-full tw-text-sm tw-text-ellipsis'
                  >
                    {
                      card.name
                    }
              ...
              <Box
                className='tw-max-w-full tw-mt-3 tw-flex'
              >
                {
                  card.description &&
                    <Tooltip
                ...
                }
                {
                  (card.dueDate || card.startDate) &&
                    <Tooltip
                        title='Есть ограничение по времени'
                        placement='bottom'
                    >
                        <Box
